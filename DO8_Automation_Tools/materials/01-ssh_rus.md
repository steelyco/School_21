...
**SSH (Secure shell)** — безопасный протокол прикладного уровня для подключения к удаленной машине. SSH является основным способом удаленного подключения к компьютерам с Unix-подобной ОС.

Чтобы подключиться к машине по SSH, можно использовать два подхода:
